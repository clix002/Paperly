...
export const useAutoResize = ({ canvas, container, canvasBgRef }: UseAutoResizeProps) => {
  ...
  const autoZoom = useCallback(() => {
    ...
    canvas.backgroundColor = canvasBgRef?.current ?? "#e5e5e5"
    canvas.requestRenderAll()
  }, [canvas, container, canvasBgRef])
