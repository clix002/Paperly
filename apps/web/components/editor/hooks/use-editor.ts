...
async function loadPageIntoCanvas(canvas: Canvas, pageData: object, bg?: string) {
  await canvas.loadFromJSON(pageData)
  canvas.backgroundColor = bg ?? CANVAS_BG
}
...
const buildEditor = ({
  ...
  const loadJson = async (json: string | object) => {
    ...
    await loadPageIntoCanvas(canvas, firstPage, canvasBgRef?.current)
    autoZoom()
    save()
  }
  ...
  return {
    ...
    zoomIn: () => {
      ...
      applyZoom(canvas, container, workspace, zoomRatio, canvasBgRef?.current)
    },
    zoomOut: () => {
      const workspace = getWorkspace() as Rect
      if (!workspace || !container) return
    ...
    addImage: async (value: string) => {
      ...
      const workspace = getWorkspace()
      const maxWidth = (workspace?.width || 400) * 0.4
...
export const useEditor = ({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: canvasBgRef is a stable ref
  const addPage = useCallback(async () => {
    ...
    await loadPageIntoCanvas(canvas, newPageContent, canvasBgRef?.current)
  ...
  const deletePage = useCallback(
    async (index: number) => {
      ...
      if (pagesArray[0] && canvas) {
        await loadPageIntoCanvas(canvas, pagesArray[0], canvasBgRef?.current)
        autoZoom()
      }
    },
    [canvas, autoZoom]
  ...
  const changePage = useCallback(
    async (index: number) => {
      ...
      canvas.clear()
      await loadPageIntoCanvas(canvas, pageContent, canvasBgRef?.current)
      autoZoom()
  ...
  const init = useCallback(
    ({
      ...
      initialCanvas.backgroundColor = canvasBgRef?.current ?? CANVAS_BG
